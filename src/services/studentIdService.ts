...
export const fetchLatestStudentId = async (): Promise<string> => {
  try {
    ...
    if (response.status === 200) {
      ...
      const academicBackground = useAcademicStore.getState().academicBackground;
      const yearEntry = academicBackground?.yearEntry
        ? academicBackground.yearEntry.toString().slice(-2)
        : null;
            const departmentId = useAcademicStore.getState().academicBackground.department;
      if(students.length === 0 && yearEntry!== null){
